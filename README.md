...
This repo can now run in two first-class modes:

- web server mode under `/webterm/`
...
- The browser app is served from `/webterm/`, and all web routes are prefixed with `/webterm/`.
...
- Drag-and-drop uploads still write into `.webterm_uploads` and paste the resulting path into the terminal.
